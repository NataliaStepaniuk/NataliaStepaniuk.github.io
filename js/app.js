...
function Cart(tax = 0.0, shipping = 0) {
  ...
  this.populateShoppingCart = (products) => {
    ...
    cart.forEach(
      (item) => (result += cartItemTemplate(item, findItem(products, item.id)))
    );
    return result;
  };
  ...
  this.setCartTotal = function (shoppingCartItems) {
    ...
    cart.map((item) => {
      let price = shoppingCartItems.querySelector(
        `#id${item.id} .product-price`
      ).textContent;
      tmpTotal = +price * item.amount;
      shoppingCartItems.querySelector(
        `#id${item.id} .product-subtotal`
      ).textContent = parseFloat(tmpTotal).toFixed(2);
...
function ProductList(products) {
  ...
  this.productTemplate = (product) => `
        <article class="product" data-id="${product.id}">
            ...
            <div class="image">
                <div class="badge bg-${product.badge.bg}">${
    product.badge.title
  }</div>
            ...
            <div class="content" data-id="${product.id}">
                ...
                <span><span class="price"></span><span class="price product-price">${
                  product.price
                }</span></span> <span class="starf">${starsTemplate(
    product.stars
  )}</span>
            </div>                    
        </article>`;
...
const liElement = (item) =>
  `<li><a class="category-item" href="#!" data-id="${item.id}">${item.name}</a></li>`;
...
const renderShowOnly = (showOnly, products, productContainer) => {
  let badges = [
    ...new Set(
      [...products.map((item) => item.badge.title)].filter((item) => item != "")
    ),
  ];
  ...
  checkboxes.forEach((item) => {
    item.addEventListener("change", (e) => {
      if (e.target.checked) {
        ...
        productContainer.innerHTML = values
          .map((value) => renderList(products, value))
          .join("");
      } else {
        if (values.length != 0) {
...
blogPosts.forEach((post) => {
  ...
  content.classList.add("blog-collapsed");

  function toggleArticle() {
    content.classList.toggle("blog-collapsed");

    if (content.classList.contains("blog-collapsed")) {
